...
clear_wireless_config_ifname() {
	wireless_config_ifname=`cat /var/state/wireless | grep ifname | awk -F '=' '{print $1}'`
	wireless_config=`uci show | grep wireless | grep "device='radio"`

	for wireless_ifname in $wireless_config_ifname
	do
		radio=`echo $wireless_ifname | awk -F '.' '{print $2}'`
		[ -z "$(echo $wireless_config | grep $radio)" ] && uci -q -P /var/state revert $wireless_ifname
	done
}
